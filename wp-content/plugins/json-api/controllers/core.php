...
class JSON_API_Core_Controller {
  ...
  **/
  public function get_app_index(){
	...
	$topslideposts = $json_api->introspector->get_posts($args); 
	$topslideresult = $this->posts_result($topslideposts);
	...
	$newsposts = $json_api->introspector->get_posts($args); 
	$newsresult = $this->posts_result($newsposts);
	...
	$moveposts = $json_api->introspector->get_posts($args); 
	$moveresult = $this->posts_result($moveposts);
	...
	$picposts = $json_api->introspector->get_posts($args); 
	$picresult = $this->posts_result($picposts);
	...
	$techposts = $json_api->introspector->get_posts($args); 
	$techresult = $this->posts_result($techposts);
	
	
	return array(
      'topslideposts' => $topslideresult,
	  'newsposts' => $newsresult,
	  'moveposts' => $moveresult,
	  'picposts' => $picresult,
	  'techposts' => $techresult,
    );
  }
  ...
  public function get_top_posts(){
      ...
      $posts = $json_api->introspector->get_posts($top_args);
      $result = $this->posts_result($posts);
      return $result;
  }
     ...
     * 获取服务器当前毫秒的时间戳
     **/
  public function get_server_mtime(){
    list($t1, $t2) = explode(' ', microtime());
    $time = array();
    $time['time'] = (float)sprintf('%.0f',(floatval($t1)+floatval($t2))*1000);
    return $time;
  }
