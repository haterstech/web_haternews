...
$hc_file = '/data/hosts/bballman/wp-content/cache/hyper-cache/' . $hc_host . $hc_uri . '/index' . $hyper_cache_group . '.html';
...
if (0 && $hyper_cache_is_bot) {
    ...
    if (5 > 0 && $hc_file_time < time() - (5 * 3600)) {
...
if (1) {
    if (2 != 0) {
        $hc_cache_max_age = 2 * 3600;
    } else {
        // If there is not a default expire time, use 24 hours.
        if (5 > 0) {
            $hc_cache_max_age = time() + (5 * 3600) - $hc_file_time;
        } else {
